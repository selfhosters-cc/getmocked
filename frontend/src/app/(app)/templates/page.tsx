...
export default function TemplatesPage() {
  ...
  const handleRename = async (id: string) => {
    ...
    await api.updateSiteTemplate(id, { name: renameValue.trim() })
        ...
        <div className="flex items-center gap-3">
          <div className="relative">
            ...
            <input
              ...
              className="pl-9 pr-4 py-2 border rounded-lg text-sm w-48 md:w-64 focus:outline-none focus:ring-2 focus:ring-blue-500"
            />
          </div>
          {isAdmin && (
            <button
      ...
      {loading && images.length === 0 ? (
        ...
        <div className="flex flex-col items-center justify-center py-20 text-gray-400">
          ...
          <p className="text-sm mt-1">
            {search ? 'Try a different search term' : 'Check back later for new templates'}
          </p>
        </div>
      ) : (
        <>
